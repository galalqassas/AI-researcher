...
Auto-Researcher is a Python application that automatically ingests research papers from arXiv, classifies them into research buckets (General AI, Autonomous Agents, AI+Finance), and generates plain-English summaries via a local/cloud LLM (Ollama). It provides a FastAPI JSON API and a Click CLI for pipeline operations. Vector embeddings are stored in **Pinecone** (Docker) alongside SQLite metadata.
...
- **Vector Database:** Pinecone (Docker, `pinecone-client==1.18.0`) — supports gRPC transport
...
├── docker-compose.yml      # Pinecone vector database service
...
│   │   ├── pinecone_store.py # Pinecone client: collection management, upsert, search, delete, resync
...
| `QDRANT_GRPC_PORT` | `6334` | Pinecone gRPC API port (used for faster batch operations) |
...
- **Paper** (`papers` table): id, arxiv_id (unique), title, authors (nullable), abstract (nullable), full_text (nullable), pdf_url (nullable), published_date (nullable Date), ingested_at (nullable DateTime), buckets (nullable JSON string), embedding (nullable LargeBinary — also synced to Pinecone)
...
- Pinecone client is a module-level singleton (`_pinecone_client` in `pinecone_store.py`) with gRPC transport enabled by default. All functions handle `None` client gracefully.
...
pinecone-client==1.18.0              # Vector database client (REST + gRPC)
click                              # CLI
...
docker compose up -d              # Start Pinecone vector database
